...
> **新数据(§2.9 之后才测得了)**:真权重 28 层,`REPEAT` 斜率隔离设备时间。
> **一个 step 之内,代价严格线性于 token 数**(5→10 个 token:8.37→16.20 s,1.94×,
> 约 **1.6 s/token**);**但把同样的工作拆成两个 step 要贵 3.8×**
> (10 个 token 位置:一个 step 16.20 s,两个 step 60.87 s)。
> 3.8× 是 `steps>1` 的**一次性**惩罚,不随 step 数增长。
> **原因我没查出来,也没瞎猜**:验过两个假设都不成立 —— 单趟 step 循环并没有被特化
> (gpu.module 在 steps=1/2 下是 3493/3494 行),设备侧也没有任何循环上界随 `steps` 变大。
> 要查清得先修下面那条 kernel 内计时(或者照 Fleet 的 `MPK_DEVICE_TIMING` 用
> `s_memrealtime` 累加,更便宜)。
> **对外引用要用多步那个数:约 6.4 s/token**,对比 Fleet torch baseline 26.57 ms/token
> 是 **~240×**。旧文件里那个 1.67 s/token 是 `steps=1` 行,正好乐观了这 3.8×。
